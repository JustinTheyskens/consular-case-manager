...
export const UserDashboard = () => {
  ...
  const userId = useSelector(
    (state: { session: SessionState }) => state.session.userId,
  );
  const name = useSelector(
    (state: { session: SessionState }) => state.session.name ?? "guest",
  );

  // API hooks
  const { data: cases, isLoading: casesLoading } = useGetCasesByCitizenQuery(
    userId!,
    {
      skip: !userId,
    },
  ...
  const [selectedAppointment, setSelectedAppointment] =
    useState<AppointmentType>(appointmentTypes[0]);
    ...
    <Box>
      <ThemeProvider theme={theme}>
        <Box minHeight="100vh">
          ...
          <Box sx={{ px: 3, pb: 4 }}>
            {/* Summary Metrics */}
            <Grid container spacing={3} justifyContent="right" sx={{ mb: 8 }}>
                          ...
                          <>
                            ...
                            >
                              ...
                              <Typography variant="body2" fontWeight={500}>
                                {upcomingAppointment.appointment.type}
                              </Typography>
                            </Box>
                            <Box
                              sx={{
